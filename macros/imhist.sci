...
function [counts, cells] = imhist(im, varargin)
  ...
  select imtype
   case 'boolean' then
    if isempty(pin) then, pin=2; end
    cells = linspace(0,1,pin);
   case 'uint8' then
    if isempty(pin) then, pin=2^8; end
    cells = linspace(0, 255, pin)
   case 'int8' then
    if isempty(pin) then, pin=2^8; end
    cells = linspace(-128, 127, pin)
   case 'uint16' then
    if isempty(pin) then, pin=2^16; end
    cells = linspace(0, 2^16-1, pin)
   case 'int16' then
    if isempty(pin) then, pin=2^16; end
    cells = linspace(-2^15, 2^15-1, pin)
   case 'int32' then
    if isempty(pin) then, 
      pin=2^16; 
      printf("The number of bins is set to 2^16 for int32 images."); 
    end
    cells = linspace(-2^31, 2^31-1, pin)
   case 'constant' then
    if isempty(pin) then, pin=10; end
    cells = linspace(0, 1, pin)
  else
