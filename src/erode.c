...
int int_erode(char *fname)
{
  ...
  if (Rhs==2)
	...
	if (flag==0)
	  {
	    truc=cvCreateStructuringElementEx( a[0], b[0], c[0], d[0], CV_SHAPE_RECT,NULL );
	  }
	...
	cvErode(img1,img2,truc,nbr_it[0]);
	//transform the result of opencv canny in a matrice
	IplImg2Mat(img2, 8);
	//send the result
	LhsVar(1)=8;	
