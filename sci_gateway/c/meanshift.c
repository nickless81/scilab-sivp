 ...
 * [object]=meanshift(im) or [object]=meanshift(im,window)
 * object is a 1x4 matrix of the object window [x,y,w,h]
...
int int_meanshift(char *fname)
{
  ...
  pObject = cvCreateImage(cvSize(4, 1), IPL_DEPTH_64F, 1);
  if(pObject == NULL)
  {
     cvReleaseImage(&pSrcImage);
     cvReleaseImage(&pHSVImage);
  ...
  if(Rhs == 1)
   {
    ...
    //meanshift tracking
    cvMeanShift(pBPImage, object_window, cvTermCriteria(CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 10, 1), &comp); 
    //object window
